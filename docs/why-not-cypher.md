...
The last of those is a design property rather than a measured one, and it is worth stating precisely. theorem verifies a whole query against the live schema before executing any of it and Cypher has no such step, so on 3,925 mutated queries theorem refuses 1,811 of 1,928 and text2cypher refuses none of 1,997. That contrast follows from the two language definitions; running it did not establish it, and it should not be quoted as evidence. What the same exercise did establish is theorem's own limit: 6.1% of its broken queries still return an answer, every one of them a reversed role on an edge whose two roles hold the same class, which no schema check can distinguish. The tables are on the [broken-query page](benchmarks/silent-failure.md).
