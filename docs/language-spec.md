...
**Scale.** Everything is in memory, with no spill to disk, at roughly 6.7 KB per node measured on the CypherBench `politics` graph (885k nodes, 5.7 GB). A query's binding table is built on top of that, so the envelope is two thirds of RAM rather than all of it: about 3.4 million nodes on a 32 GB machine. `theorem stats --db <path>` reports where a store sits against it. On the CypherBench `soccer` graph, 275,185 nodes and 1,119,766 edges in 1.7 GB: a lookup by name 0.03 ms, one hop from it 0.03 ms, two hops 0.5 ms, a grouped count over a class 23 ms, a global count 124 ms, and a filter on a property other than `name` 288 ms. Reads write nothing to the log.

**Indexes.** `name` is always indexed. Any other property is indexed the first time a query filters on it with `=`, and only on a class of at least 5,000 nodes, below which a scan is cheaper than the memory an index costs. The first such query pays the scan; every one after it does not. On the 237k-node `player` class that is 299 ms against 20 ms for a common value, 341 ms against 0.8 ms for a rare one, and 304 ms against nothing at all for a value no node has.

An index is only ever a prefilter: the condition is still evaluated on every candidate it returns, so an index can narrow the work and cannot change the answer. Exceeding it fails on the next question rather than the next node, which is why the figure has headroom in it.
