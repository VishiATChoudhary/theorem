...
**Scale.** Everything is in memory, with no spill to disk, at roughly 6.7 KB per node measured on the CypherBench `politics` graph (885k nodes, 5.7 GB). A query's binding table is built on top of that, so the envelope is two thirds of RAM rather than all of it: about 3.4 million nodes on a 32 GB machine. `theorem stats --db <path>` reports where a store sits against it. On the CypherBench `soccer` graph, 275,185 nodes and 1,119,766 edges in 1.7 GB, a lookup by name and a two-hop traversal from it are sub-millisecond, a grouped count over a class is tens of milliseconds, and anything that scans a class (a global count, or a filter on a property other than name) is hundreds. Reads write nothing to the log. Only `name` is indexed; a filter on any other property is a scan of the class. Exceeding it fails on the next question rather than the next node, which is why the figure has headroom in it.
