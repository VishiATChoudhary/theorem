...
\subsection{Broken queries (new, and not a benchmark)}
...
\textbf{That comparison is definitional, and it has been demoted accordingly.}
theorem checks a whole query against the live schema before executing any of it;
Cypher has no such step. The 1{,}811-versus-0 split therefore follows from the
two language definitions, and running 3{,}925 mutants did not establish it. The
four mutations are ours as well, chosen and applied uniformly rather than sampled
from how models actually break queries, so the mix is a guess and a different mix
...
guaranteed by the design. It found a blind spot that had been assumed away:
...
\begin{itemize}
  ...
  \item \textbf{A mutation is not a model.} This measures what a language does
    with a broken query, not how often a model breaks one. That is execution
    accuracy, which is a real benchmark, and it is where the case for the
    language belongs.
  \item \textbf{Refusing is not answering.} A language that refused every query
    would score 0\% undetectable here. The figure is meaningless without an
    accuracy figure beside it.
\end{itemize}
...
uv run python -m eval.run_silent --graph nba  # broken queries (design note)
...
Prompt cost, tokens per class       & \textbf{39} & 85 \\
\bottomrule
\end{tabular}
\end{center}
...
tested directly and does not hold.

The claim the project is actually built on, that a wrong query is loud rather
than plausible, is \emph{not} in that table. The mutation study meant to
establish it turned out to restate the design rather than test it, so it is
reported as a design note (Section~2.3). Its one real result is a limit on this
language rather than a win over another: 6.1\% of deliberately broken theorem
queries still return an answer, every one of them a reversed role on an edge
whose two roles hold the same class.
