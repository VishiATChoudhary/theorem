...
class WriteContext:
    store: Store
    schema: Schema
    env: dict[str, object] = field(default_factory=dict)  # name -> node id | list[node id]
    ...
    def resolve_ref(self, ref: str) -> str:
        ...
        if isinstance(value, list):
            raise WriteError(f'"{ref}" is bound to {len(value)} nodes; need exactly one')
...
def _assert_node(stmt: AssertNode, ctx: WriteContext) -> Receipt:
    ...
    record = {"op": "put_node", "id": nid, "cls": stmt.cls, "props": props,
              "state": state}
    if stmt.source:
    ...
    display = [{**c, "name": store.nodes[c["a"]].props.get("name", c["a"])}
               for c in cands]
    return Receipt(
...
def _assert_edge(stmt: AssertEdge, ctx: WriteContext) -> Receipt:
    ...
    for role, nid in roles.items():
        ...
        if want not in base_chain:
            raise WriteError(f"role {role} of {stmt.edge} takes a {want}; {nid} is a {got}")
    ...
    return Receipt([f"receipt: created edge {stmt.edge}({role_str}) = {eid} at @t-{pos}",
...
def _merge(stmt: Merge, ctx: WriteContext) -> Receipt:
    ...
    pre_states = {
        n.id: {"props": {k: v for k, v in n.props.items() if not k.startswith("_")},
               "state": n.state, "created_at": n.created_at}
        for n in (na, nb)}
    ...
    store.apply({"op": "lineage", "kind": "merge", "survivor": survivor.id,
                 "absorbed": absorbed.id, "pre_states": pre_states,
                 "policy": stmt.policy})
    ...
    for edge in list(store.edges.get(absorbed.id, [])):
        if edge.retired_at is not None:
            continue
        new_roles = {r: (survivor.id if store.resolve(nid) == absorbed.id else nid)
                     for r, nid in edge.roles.items()}
        store.apply({"op": "retire_edge", "id": edge.id})
        store.apply({"op": "put_edge", "id": store.next_id("edge"),
                     "type": edge.type, "roles": new_roles})
    store.apply({"op": "alias", "absorbed": absorbed.id, "survivor": survivor.id})
    pos = store.apply({"op": "patch_node", "id": survivor.id, "props": merged})
    store.apply({"op": "retire", "id": absorbed.id, "reason": f"merged into {survivor.id}"})
    return Receipt([f"receipt: merged -> {survivor.id} (lineage keeps both) at @t-{pos}",
...
def _distinct(stmt: Distinct, ctx: WriteContext) -> Receipt:
    ...
    ctx.store.dup_ledger[:] = [
        r for r in ctx.store.dup_ledger
        if frozenset((ctx.store.resolve(r["a"]), ctx.store.resolve(r["b"])))
        != frozenset((a, b))]
    return Receipt([f"receipt: distinct {a}, {b} recorded at @t-{pos}",
...
def _refine(stmt: Refine, ctx: WriteContext) -> Receipt:
    ...
    for target, source_col in stmt.mapping.items():
        ...
        if source_col not in rows[0]:
            raise WriteError(f"payload has no column {source_col!r}; "
    ...
    for row in rows:
        ...
        store.apply({"op": "put_node", "id": cid, "cls": stmt.into_cls,
                     "props": props, "state": "atom", "origin": blob_id})
        store.apply({"op": "lineage", "kind": "refine", "parent": blob_id,
                     "child": cid, "mapping": stmt.mapping})
        child_ids.append(cid)
    pos = store.apply({"op": "patch_node", "id": blob_id, "props": {},
                       "state": "composite"})
    n_dups = 0
    ...
    lines = [f"receipt: refined {blob_id} -> {len(child_ids)} {stmt.into_cls} nodes at @t-{pos}",
...
def _compact(stmt: Compact, ctx: WriteContext) -> Receipt:
    ...
    store.apply({"op": "put_node", "id": sid, "cls": cls,
                 "props": dict(stmt.props), "state": "composite"})
    for m in members:
        mid = store.resolve(m)
        store.apply({"op": "lineage", "kind": "compact", "parent": sid,
                     "child": mid})
        store.apply({"op": "retire", "id": mid, "reason": f"compacted into {sid}"})
    pos = store.position
    ctx.env[stmt.name] = sid
    return Receipt([f"receipt: compacted {len(members)} {cls} nodes -> {sid} at @t-{pos}",
...
def _retire(stmt: Retire, ctx: WriteContext) -> Receipt:
    ...
    return Receipt([f"receipt: retired {nid} at @t-{pos}",
...
def _flag(stmt: Flag, ctx: WriteContext) -> Receipt:
    ...
    return Receipt([f"receipt: flagged {nid} at @t-{pos}",
                    "  health.query updated"])


def _derive(stmt: DeriveClass, ctx: WriteContext) -> Receipt:
    schema = ctx.schema
    schema.classes[stmt.name] = ClassDef(
        name=stmt.name, props=dict(stmt.props), base=stmt.base,
        status="provisional", quota=PROVISIONAL_QUOTA)
    pos = ctx.store.apply({"op": "lineage", "kind": "derive_class",
                           "child": stmt.name, "parent": stmt.base,
                           "props": stmt.props})
    lines = [f"receipt: class {stmt.name} provisional at @t-{pos}",
    ...
    if similar:
        ...
        lines.append(f'  note: similar existing class "{similar[0]}" '
