...
def test_assert_receipt_and_dup_candidate(wctx):
    (r1,) = run_writes(
        'assert part {name: "graphene sheet", unit_cost: 12.0} source doc:d/p3 as gs',
        wctx,
    )
    out = r1.render()
    ...
    (r2,) = run_writes(
        'assert part {name: "graphene sheets", unit_cost: 11.8} source doc:d/p4 as gs2',
        wctx,
    )
    out2 = r2.render()
...
def test_assert_edge_via_binding(wctx):
    ...
    (r,) = run_writes(
        f"assert edge supplied_by(item: gs, source: {wctx.store.ids['volta']})", wctx
    )
...
def test_merge_unwindable_state_recorded(wctx):
    ...
    rec = next(r for r in store.lineage if r["kind"] == "merge")
...
def test_distinct_suppresses(wctx):
    ...
    ctx = ReadContext()
    out = execute_read(
        verify(parse("find dup_candidates as d\nreturn d.a, d.b"), wctx.schema),
        store,
        wctx.schema,
        ctx,
    )
    ...
    receipts = run_writes(
        'assert supplier {name: "VoltaChem GmbH", country: "DE"} as v3', wctx
    )
    assert all(
        frozenset((a, b)) != frozenset((c["a"], c["b"]))
...
def test_refine_csv_blob(wctx, tmp_path):
    ...
    att_dir.mkdir()
    (att_dir / "csv-1.csv").write_text(
        "component,eur_unit\nbolt,0.1\nnut,0.05\nscrew,0.2\n"
    )
    run_writes(
        'assert table_blob {title: "prices", payload: attach:csv-1} source doc:r as prices',
        wctx,
    )
    (r,) = run_writes(
    ...
    blob_id = next(n.id for n in store.nodes.values() if n.cls == "table_blob")
...
def test_compact(wctx):
    ...
    plans = verify(
        parse(
            'find part where unit_cost < 3 as cheap\ncompact cheap as summary {name: "cheap parts 2026", unit_cost: 1.5}'
        ),
        wctx.schema,
    )
    ...
    wctx.env["cheap"] = [
        n.id
        for n in wctx.store.nodes.values()
    ...
    summary = [
        n
        for n in wctx.store.nodes.values()
        if n.props.get("name") == "cheap parts 2026"
    ]
...
def test_derive_class_provisional(wctx):
    ...
    run_writes(
        'assert distributor {name: "EuroParts", country: "FR", region: "EU"} as d', wctx
    )
